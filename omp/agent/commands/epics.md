...
Discover and read the active `/epic` command using OMP's resolution order: a repository-scoped `.omp/commands/epic.md` when present, otherwise `~/.omp/agent/commands/epic.md`. If neither exists, stop with the missing prerequisite. A repository-scoped command may add constraints but must preserve the managed model pins, exclusive Flash writing, isolated task items, read-only dual review, lease liveness, and durable checkpoints; return `BLOCKED` on any weakened or contradictory rule. Apply the validated repository discovery, reconciliation, implementation, independent review, remediation, final-gate, safety, lease, and durable-checkpoint rules independently to every epic below. This command only adds portfolio scheduling.
