...
- Delegate every application-code, test, migration, generated-code, and remediation edit exclusively to `luna-fast` (GPT-5.6 Luna max). When a change is too complex for one bounded Luna task, Sol decomposes it into smaller contracts; it never hands code writing to another model.
- For every meaningful delivered change, the Sol control plane first executes the prescribed verification and freezes an exact pushed SHA, then launches fresh `sol-reviewer` (GPT-5.6 Sol medium) and `terra-pro` (GPT-5.6 Sol high adversarial) task items independently with `isolated: true`. Both source-only reviews are required before the completion claim.
